...
def GetBtcBalance():
    ...
    reqUrl = f"https://tbtc1.trezor.io/api/v2/address/{address}"
...
def BroadcastBtcTransaction():
    ...
    resp = requests.post("https://tbtc1.trezor.io/api/v2/sendtx", params = {"rawhex": rawHex})
...
def GetBtcUTXO():
    address = request.args.get("address")
    reqUrl = f"https://tbtc1.trezor.io/api/v2/utxo/{address}"
    resp = requests.get(reqUrl)
...
def GetBtcTransaction():
    ...
    reqUrl = f"https://tbtc1.trezor.io/api/v2/address/{address}?page={offset + 1}&pageSize={limit}"
    resp = requests.get(reqUrl)

    txids = resp.json().get("txids")
    for txid in txids:
        reqUrl = f"https://tbtc1.trezor.io/api/v2/tx/{txid}"
        resp = requests.get(reqUrl)
        print(resp.json())
